...
class Program
{
        ...
        while (choice != 6)
        {
            ...
            if (choice == 1)
            {
                ...
                else if (choiceTwo == 4)
                {
                    ...
                    negativeGoal.SetGoalName();
                    negativeGoal.SetGoalDescription();
                    Console.WriteLine("When entering the points enter it as a positive and do not use a - sign.");
                    negativeGoal.SetGoalPoints();
            ...
            else if (choice == 3)
            {
                ...
                using (StreamWriter outputFile = new StreamWriter(fileName))
                {
                    ...
                    foreach (Goals goal in goals)
                    {
                        ...
                        {
                            outputFile.WriteLine($"Negative Goal,{goal.GetGoalName()},{goal.GetGoalDescription()},{goal.GetGoalPoints()},{goal.GetGoalFinished()}");                  
                        }
                    }
                }
            }
            else if (choice == 4)
            {
                ...
                {   
                    ...
                    else if (parts[0] == "Negative Goal")
                    {
                        NegativeGoal negativeGoal = new NegativeGoal(parts[1],parts[2],int.Parse(parts[3]),parts[4]);
                        goals.Add(negativeGoal);
            ...
            else if (choice == 5)
            {
                ...
                else if (goalAccomplished is NegativeGoal)
                {
                    goalAccomplished.SetGoalFinished("X");
                    totalPoints -= goalAccomplished.GetGoalPoints();
                }
    ...
    /* What I have done to exceed the requirements is I have added a Negative Goal. The user can add a negative goal
    and if they do the negative goal then it will subtract points from their total points.*/
}
